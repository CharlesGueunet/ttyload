...
char strbuf[BUFSIZ], *optstring = "i:hvmsr:c:",
...
void showloads(load_list *loadavgs, int simplemode);
void clear_screen();
void home_screen();
...
void print_header(int current) {
  printf("%s   %.2f, %.2f, %.2f   %s       ttyload, v%s\n\n", hostname,
         ...
         (loadavgs[current].fifteen_minute / 1024.), strbuf, version);
}

void update_clocks(time_t thetime, struct tm *thetimetm, int position) {
  int lastclock = (theclock + clocks - 1) % clocks;

  /* if we don't have a previous clock, or "enough" time has past to have
   * another */
  if (theclocks[lastclock].pos < 0 /* getting-started special */ ||
      (((thetime - theclocks[lastclock].when) >=
        CLOCKWIDTH * intsecs) && /* enough visual buffer space */
       ((thetime / 60) > (theclocks[lastclock].when /
                          60)))) /* at least a minute (for different value) */
  {
...
{
  load_list newload;
  int c, i, errflag = 0, versflag = 0, simplemode = 0;
  char *basename;
  time_t thetime;
  struct tm *thetimetm;
  ...
  for (i = 0; i < width;
       ++i /* note: gets decremented, too; this is forever! */) {
    if (i != 0) {
      sleep(intsecs);
    }
...
void showloads(load_list *loadavgs, int simplemode) {
  ...
  if (debug > 3) {
    printf("Starting with min set: %ld, %ld, %ld\n", min.one_minute,
           min.five_minute, min.fifteen_minute);
    printf("Starting with first set: %ld, %ld, %ld\n", loadavgs[0].one_minute,
           loadavgs[0].five_minute, loadavgs[0].fifteen_minute);
    sleep(1);
  }
  for (i = 0; i < width; i++) {
    if (debug > 9) {
  ...
  if (debug > 3) {
    ...
    printf("MIN Load averages: %f, %f, %f\n", min.one_minute / 1024.,
           min.five_minute / 1024., min.fifteen_minute / 1024.);
    printf("MAX Load averages: %f, %f, %f\n", max.one_minute / 1024.,
           max.five_minute / 1024., max.fifteen_minute / 1024.);
  }
  ...
  for (i = 0; i < width; i++) {
    loadavgs[i].height1 = compute_height(loadavgs[i].one_minute, lmax, height);
    loadavgs[i].height5 = compute_height(loadavgs[i].five_minute, lmax, height);
    loadavgs[i].height15 =
        compute_height(loadavgs[i].fifteen_minute, lmax, height);

    if (debug > 3) {
      printf("Load averages: %f, %f, %f  -- ", loadavgs[i].one_minute / 1024.,
             loadavgs[i].five_minute / 1024.,
             loadavgs[i].fifteen_minute / 1024.);
      printf("Heights: %d, %d, %d\n", loadavgs[i].height1, loadavgs[i].height5,
             loadavgs[i].height15);
    }
  }

  for (j = 0; j <= height; j++) {
    printf("%6.2f   ", (((omax) * (height - j)) / (height)));
    for (i = 0; i < width; i++) {
      k = 0;
  ...
  for (i = 0; i < clocks; i++) {
    if (theclocks[i].pos >= 0) {
      strncpy(&strbuf[9 + theclocks[i].pos], theclocks[i].clock, 6);
    }
  }
  ...
  if (!simplemode) {
    printf("%s\n  Legend:\n"
           ...
           strbuf, loadstrings[1], loadstrings[2], loadstrings[4],
           loadstrings[3], loadstrings[5], loadstrings[6], loadstrings[7]);
  }
}

int compute_height(thisload, maxload, height) load_t thisload, maxload;
int height;
{
...
void home_screen() { printf("\033[H"); }

void cycle_load_list(loadavgs, newload, width) load_list *loadavgs, newload;
int width;
{
